...
HTMl 파일 안에서 꺽쇠 괄호 등 일부 기호를 사용할 때, 그것이 html 태그 기호 등 시스템적으로 가진 기능으로 인식될 수 있어 렌더링하기 모호하다. 이 때 각 기호('<')를 HTML 엔티티(&lt)로 변환. 이 변환을 html 이스케이핑이라고 부른다.

Django 템플릿은 기본 설정상 자동으로 html 이스케이프 처리를 수행함.
